//------------------------------------------------------------
// 2. uart_rx_overrun_test
//------------------------------------------------------------
class uart_rx_overrun_test extends uvm_test;
  `uvm_component_utils(uart_rx_overrun_test)

  apb_uart_env env;
  apb_write_seq w_baud, w_ctrl;
  uart_inject_seq rx_data[$];  // RX FIFO writes
  ...
  task run_phase(uvm_phase phase);
    ...
    `uvm_info("TEST", "Running UART RX Overrun test", UVM_LOW)

    w_baud.addr = ADDR_BAUDDIV; w_baud.data = 32'd27;
    w_baud.start(env.apb_ag.m_sequencer);

    w_ctrl.addr = ADDR_CTRL; w_ctrl.data = 32'h2; // RXEN only
    ...
    `uvm_info("UART_RX_OVERRUN", "Check STATUS for RX overrun flag", UVM_LOW)
